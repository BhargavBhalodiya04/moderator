...
const SignalsPage: React.FC<{ showResolved?: boolean }> = ({
  ...
  useEffect(() => {
    ...
    if (maxUpdatedAt(issuesData?.content ?? []) > baseline) {
      trackEvent(ModeratorEvent.DIAGNOSTICS_RUN_COMPLETED, {
  ...
  useEffect(() => {
    ...
    if (failedAt > failBaseline) {
      trackEvent(ModeratorEvent.DIAGNOSTICS_RUN_FAILED, {
        project_id: projectId,
  ...
  const handleRunDiagnostic = () => {
    trackEvent(ModeratorEvent.DIAGNOSTICS_RUN_CLICKED, {
      project_id: projectId,
  ...
  const handleTurnOnAuto = () => {
    trackEvent(ModeratorEvent.DIAGNOSTICS_AUTO_ENABLED, {
      project_id: projectId,
      source: "header",
    });
