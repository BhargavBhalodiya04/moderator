...
import { ModeratorEvent, trackEvent } from "@/lib/analytics/tracking";
import {
  useAgentOnboarding,
...
const ConnectAgentStep: React.FC = () => {
  ...
  // Variants: "control" = AI-assisted tab shows "Install with AI" (Moderator skills prompt); "connect-to-ollie" = AI-assisted tab shows "Connect to Ollie"; "manual" = bypasses this modal entirely and renders the full integrations page (handled in NewQuickstart). Undefined falls back to "connect-to-ollie".
  const variant =
    useFeatureFlagVariantKey(AI_ASSISTED_OPIK_SKILLS_FEATURE_FLAG_KEY) ??
    DEFAULT_ONBOARDING_FLOW;

  const aiAssistedUsesModeratorSkills = variant === "control";
  const showOllieTab = !!apiKey && !aiAssistedUsesModeratorSkills;
  ...
  useEffect(() => {
    if (firstTraceId && firstTraceId !== trackedTraceId) {
      trackEvent(ModeratorEvent.ONBOARDING_FIRST_TRACE_RECEIVED, {
  ...
  return (
    <AgentOnboardingCard
      title={`Set up Moderator for ${agentName}`}
      description="Connect your repo so Moderator can help set up tracing, or instrument your code manually."
      headerContent={
