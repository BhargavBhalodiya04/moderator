...
const McpAnnouncementBanner: React.FC<McpAnnouncementBannerProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    trackEvent(ModeratorEvent.MCP_BANNER_SHOWN, bannerEventProperties(copyVariant));
  }, [countable, copyVariant]);

  const handleCtaClick = useCallback(() => {
    trackEvent(
      ModeratorEvent.MCP_BANNER_CTA_CLICKED,
  ...
  const handleDismiss = useCallback(() => {
    trackEvent(
      ModeratorEvent.MCP_BANNER_DISMISSED,
      bannerEventProperties(copyVariant),
    );
    dismiss();
  }, [dismiss, copyVariant]);
  ...
  return (
    <div
      ref={ref}
      role="region"
      aria-label="Moderator MCP announcement"
