...
const debugStep = ({
  ...
  return `4. Then read Moderator ${entity} in project "${inlineValue(
    projectName,
...
export const buildHostedInstallPrompt = (
  ...
  [
    "Connect me to Moderator MCP, then debug a failing trace.",
    "",
    DETECT_STEP,
 ...
 * once a client is named, but only while an Moderator configuration already exists;
...
export const buildLocalInstallPrompt = (
  ...
  [
    ...
    "",
    DETECT_STEP,
    `2. Install uv if it is missing, then run \`uvx opik mcp configure --ai-client <agent> --skills\` for each chosen agent. It reuses my existing Moderator configuration, which may point somewhere else: the workspace I am looking at is "${inlineValue(
