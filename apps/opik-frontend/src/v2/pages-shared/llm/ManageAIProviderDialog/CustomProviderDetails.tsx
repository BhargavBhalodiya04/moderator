        ...
        render={({ field, formState }) => {
          ...
          return (
            <FormItem>
              ...
              <Description>
                {
                  "Use `{model}` as a placeholder in the URL if your gateway expects the model name in the path — Moderator substitutes the selected model at request time. The model name is interpolated raw, so values containing `/` (e.g. HuggingFace-style names) will become extra path segments."
                }
              </Description>
            </FormItem>
          );
