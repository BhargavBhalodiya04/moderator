...
type AppStore = {
  ...
  setModeratorWorkspaceOverride: (workspaceName: string | null) => void;
};
...
const useAppStore = create<AppStore>((set) => ({
  ...
  setModeratorWorkspaceOverride: (workspaceName: string | null) =>
...
// invites, "Back to Moderator") should target. Normally this is the active
...
// on exit; generic components read `useModeratorWorkspaceName()` and stay unaware of
// the feature. When no override is set it falls back to the active workspace.
export const useSetModeratorWorkspaceOverride = () =>
  useAppStore((state) => state.setModeratorWorkspaceOverride);

export const useModeratorWorkspaceName = () =>
  useAppStore(
