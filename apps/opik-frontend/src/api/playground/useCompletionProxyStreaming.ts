...
const isModeratorError = (
  response: ChatCompletionResponse,
): response is ChatCompletionModeratorErrorMessageType => {
  return (
    "errors" in response ||
  ...
  return useCallback(
    async ({
      ...
      try {
        ...
        actualModel = response.headers.get("X-Moderator-Actual-Model");
        actualProvider = response.headers.get("X-Moderator-Provider");
        ...
        const handleModeratorErrorMessage = (
          parsedMessage: ChatCompletionModeratorErrorMessageType,
        ) => {
        ...
        while (reader) {
          ...
          for (const line of lines) {
            ...
            } else if (isModeratorError(parsed)) {
              handleModeratorErrorMessage(parsed);
