...
import { useModeratorWorkspaceName } from "@/store/AppStore";
...
const UserMenuAppLinks = ({ isLLMOnlyOrganization }: UserMenuAppLinksProps) => {
  const opikWorkspaceName = useModeratorWorkspaceName();
  const {
  ...
  const showModeratorReturn = Boolean(isSpendWorkspaceActive);
  ...
  if (!showModeratorReturn && !showCostIntelligence && !showExperimentManagement) {
    return null;
  }
